...
class OMNet(nn.Module):
    ...
    def generate_overlap_mask(self, points_src: torch.Tensor, points_ref: torch.Tensor, mask_src: torch.Tensor,
        ...
        dist_matrix = torch.sqrt(
            torch.sum(torch.square(points_src[:, :, None, :] - points_ref[:, None, :, :]), dim=-1))  # (B, N, N)
    ...
    def my_eval(self, xyz_src, xyz_ref):

        # init params
        xyz_src = torch.tensor(xyz_src).cuda()
        xyz_ref = torch.tensor(xyz_ref).cuda()

        xyz_src = xyz_src.unsqueeze(0)
        xyz_ref = xyz_ref.unsqueeze(0)
        ...
        for i in range(self.num_iter):
            ...
            # encoder
            src_encoder_feats, src_glob_feat = self.encoder[i](xyz_src_iter.transpose(1, 2).detach(),
                                                               src_pred_mask.unsqueeze(1))
            ref_encoder_feats, ref_glob_feat = self.encoder[i](xyz_ref.transpose(1, 2), ref_pred_mask.unsqueeze(1))

            # fusion
            src_cat_feat = torch.cat(
                (src_encoder_feats[0], src_glob_feat.repeat(1, 1, src_N), ref_glob_feat.repeat(1, 1, src_N)), dim=1)
            ref_cat_feat = torch.cat(
                (ref_encoder_feats[0], ref_glob_feat.repeat(1, 1, ref_N), src_glob_feat.repeat(1, 1, ref_N)), dim=1)
    ...
    def forward(self, data):
        ...
        for i in range(self.num_iter):
            ...
            overlap_src_mask, overlap_ref_mask = self.generate_overlap_mask(xyz_src.clone(), xyz_ref.clone(),
                                                                            src_pred_mask, ref_pred_mask,
                                                                            transform_gt)
